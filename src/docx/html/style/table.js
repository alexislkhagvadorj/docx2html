...
export default class Table extends Style {
  ...
  _getPropertiesConverter(category) {
    ...
    const selector = `${this.getTableSelector()}>${gRow.test(this.target) ? `.${this.getPrioritizedSelector()}>td` : `tr>.${this.getPrioritizedSelector()}`}`;
    switch (category) {
      case 'table':
  ...
  getPrioritizedSelector() {
    ...
    for (let level = this.PrioritiziedStyles.indexOf(this.target), i = 0; i < level; i++) selector = `${selector}[x${i}]`;
    return selector;
  }
}
...
Table.Properties = class Properties extends Style.Properties {
  ...
  tblCellMar(x) {
    for (const i in x) this.doc.createStyle(`${this.tableSelector}>tr>td`)[`padding${this.upperFirst(i)}`] = `${x[i] < 1 && x[i] > 0 ? 1 : x[i]}px`;// 0002
  }
...
Table.CellProperties = class CellProperties extends Style.Properties {
  ...
  tcBorders(x) {
    const tableSelector = this.parent.getTableSelector(); const
