...
const AZ = /[A-Z]/g;
const r = function (a) { return `-${a.toLowerCase()}`; };
const clozed = /Z$/gi;

function asStyle(x) {
  const a = [];
  for (const i in x) !$tool.isFunction(x[i]) && a.push(`${i.replace(AZ, r)}:${x[i]}`);
  return a.join(';');
}
...
export default class Shape extends Converter {
  ...
  convertStyle(el) {
    ...
    const propConverter = new this.constructor.Properties(el.style, this, pathStyle, bgStyle);
...
Shape.Properties = class Properties extends Style.Properties {
  ...
  gradFill(x) {
    ...
    const end = grad.pop();
    for (var i = 0, len = x.stops.length, a; i < len; i++) grad.push(`<stop offset="${(a = x.stops[i]).position}%" style="stop-opacity:1;stop-color:${a.color}"/>`);
    grad.push(end);
